...
function App() {
  ...
  const [visibleProcesses, setVisibleProcesses] = useState<Set<string>>(
    new Set(['inlet', 'prefilter', 'pump', 'aeration', 'sedimentation', 'carbon', 'disinfection', 'storage', 'outlet'])
  );
      ...
      <div className="container mx-auto px-4 py-8 h-screen flex flex-col">
        ...
        <div className="flex-1 flex gap-6 min-h-0">
          <div className="flex-1 bg-white rounded-2xl shadow-xl overflow-hidden relative">
            <Scene3D onComponentClick={handleComponentClick} visibleProcesses={visibleProcesses} />
            <div className="absolute top-4 right-4 flex gap-2">
              {selectedComponent && ['prefilter', 'aeration', 'sedimentation', 'carbon', 'disinfection'].includes(selectedComponent.type) && (
                <button
            ...
            <div className="flex-1 overflow-hidden flex flex-col">
              {activeTab === 'control' ? (
                ...
                <div className="bg-white rounded-2xl shadow-xl p-6 flex-1 overflow-y-auto space-y-4">
                  <h3 className="text-xl font-bold text-gray-800">Образовательные видео</h3>
                  <div className="space-y-3">
                    {[
                      { id: 'prefilter', name: 'Предварительная фильтрация', icon: '🔍' },
                      { id: 'aeration', name: 'Аэрация воды', icon: '💨' },
                      { id: 'sedimentation', name: 'Отстаивание', icon: '⬇️' },
                      { id: 'carbon', name: 'Угольная фильтрация', icon: '⚫' },
                      { id: 'disinfection', name: 'УФ-дезинфекция', icon: '💡' }
                    ].map(item => (
                      <button
                        key={item.id}
                        onClick={() => {
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-500">Смотреть видео</p>
                          </div>
