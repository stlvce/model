...
export default function DetailedVisualization({ componentId, onClose }: DetailedVisualizationProps) {
  ...
  const getVisualizationData = (id: string) => {
    const visualizations: Record<string, any> = {
      prefilter: {
        ...
        description: 'Механическое удаление крупных частиц размером более 50 микрон',
      ...
      carbon: {
        ...
        description: 'Адсорбция органических загрязнителей активированным углем',
    ...
    const drawPrefilter = () => {
      ...
      particlesRef.current.forEach(particle => {
        if (!isPaused) {
          ...
          if (particle.x > 390 && particle.x < 410 && particle.type === 'dirty') {
            if (Math.random() > 0.3) {
              particle.stuck = true;
        ...
        if (particle.stuck) {
          ...
          ctx.beginPath();
          ctx.arc(particle.stuckX!, particle.stuckY!, particle.size, 0, Math.PI * 2);
          ctx.fill();
        } else {
          if (particle.type === 'dirty') {
            ctx.fillStyle = particle.x < 400 ? 'rgba(180, 80, 60, 0.8)' : 'rgba(180, 80, 60, 0.3)';
          } else {
            ctx.fillStyle = particle.x < 400 ? 'rgba(100, 180, 255, 0.6)' : 'rgba(100, 200, 255, 0.8)';
          }
      ...
      particlesRef.current.forEach(particle => {
        if (!isPaused) {
          if (particle.type === 'bubble') {
            ...
            if (particle.y < 200) {
              particle.y = 550;
              particle.x = 150 + Math.floor(Math.random() * 10) * 60;
            }
          } else {
      ...
      particlesRef.current.forEach(particle => {
        if (!isPaused) {
          if (particle.type === 'sediment' || particle.stuck) {
            ...
            if (particle.type === 'dirty' && particle.y > 300 && Math.random() > 0.98) {
            ...
            if (particle.x > 700) {
              ...
              particle.type = Math.random() > 0.5 ? 'dirty' : 'water';
              particle.stuck = false;
            }
          }
        }
        ...
        if (particle.stuck) {
          ...
          ctx.fillStyle = particle.y < 300 ? 'rgba(100, 160, 220, 0.6)' : 'rgba(100, 200, 255, 0.8)';
          ctx.beginPath();
          ctx.arc(particle.x, particle.y, particle.size, 0, Math.PI * 2);
          ctx.fill();
        }
    ...
    const drawCarbon = () => {
      ...
      particlesRef.current.forEach(particle => {
        ...
        if (particle.stuck) {
          ...
          ctx.beginPath();
          ctx.arc(particle.stuckX!, particle.stuckY!, particle.size * 0.7, 0, Math.PI * 2);
          ctx.fill();
        } else {
          if (particle.type === 'dirty') {
