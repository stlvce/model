...
export default function DetailedVisualization({
    ...
    const getVisualizationData = (id: string) => {
        const visualizations: Record<string, { title: string; description: string; info: string }> =
            {
                prefilter: {
            ...
            particlesRef.current.forEach((particle) => {
                ...
                if (particle.stuck) {
                    ...
                    if (particle.type === 'dirty') {
                        ctx.fillStyle =
                            particle.x < 400 ? 'rgba(180, 80, 60, 0.8)' : 'rgba(180, 80, 60, 0.3)';
                    } else {
                        ctx.fillStyle =
                            particle.x < 400
        ...
        const drawSedimentation = () => {
            ...
            particlesRef.current.forEach((particle) => {
                ...
                if (particle.stuck) {
                    ...
                    ctx.fillStyle =
                        particle.y < 300 ? 'rgba(100, 160, 220, 0.6)' : 'rgba(100, 200, 255, 0.8)';
                    ctx.beginPath();
                    ctx.arc(particle.x, particle.y, particle.size, 0, Math.PI * 2);
                    ctx.fill();
                }
